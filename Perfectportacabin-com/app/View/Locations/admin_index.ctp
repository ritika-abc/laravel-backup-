            ...
            <tbody>
              ...
              <tr>
				...
				echo $this->Form->input('location_order', array( 'style' => 'width:50px;', 'label' => '', 'empty'=>false, 'name'=>'data[Location][location_order]', 'default'=>$location['Location']['location_order'], 'class'=>'form-control col-md-2 location_order', 'data-location_id'=>$location['Location']['id']));
 				...
 				?>
					<span class="col-md-2 success_msg_order<?php echo h($location['Location']['id']); ?>"></span>
				</td>
				<td>
				<?php				
				...
				echo $this->Form->select('active', $options, array('empty'=>false, 'name'=>'data[location][active]', 'default'=>$location['Location']['active'], 'class'=>'form-control ddStatus', 'data-location_id'=>$location['Location']['id']));
 				...
 				?>
				<span class="col-md-2 success_msg<?php echo h($location['Location']['id']); ?>"></span>
				</td>
                <td class="actions">
        ...
        $(document).on('change', '.location_order', function(e) {
            ...
            $.ajax({
                ...
                success: function(jsonResponse) {
					...
					$('.success_msg_order'+element.data('location_id')).show();
				
			    setTimeout(function() { $('.success_msg_order'+element.data('location_id')).hide(); }, 2000);
				  
				 $('.success_msg_order'+element.data('location_id')).html('<h3><i class="text-success icon-check"></i></h3>');
                    }
                });
            });            
  </script> 
        ...
        $(document).on('change', '.ddStatus', function(e) {
            ...
            $.ajax({
                ...
                success: function(jsonResponse) {
                   ...
                   // alert(response.message);
					$('.success_msg'+element.data('location_id')).show();
				
			    setTimeout(function() { $('.success_msg'+element.data('location_id')).hide(); }, 2000);
				  
				 $('.success_msg'+element.data('location_id')).html('<h3><i class="text-success icon-check"></i></h3>');
                    }
                });
            });            
  </script> 
