			...
			<?php foreach ($socialicons as $page): ?>
              <tr>
				...
				echo $this->Form->input('socialicon_order', array( 'style' => 'width:50px;', 'label' => '', 'empty'=>false, 'name'=>'data[Socialicon][socialicon_order]', 'default'=>$page['Socialicon']['socialicon_order'], 'class'=>'form-control col-md-2 socialicon_order', 'data-socialicon_id'=>$page['Socialicon']['id']));
 				...
 				?>
				<span class="col-md-2 success_msg_order<?php echo h($page['Socialicon']['id']); ?>"></span>
				</td>
				...
				echo $this->Form->select('active', $options, array('empty'=>false, 'name'=>'data[Socialicon][active]', 'default'=>$page['Socialicon']['active'], 'class'=>'form-control ddStatus', 'data-socialicon_id'=>$page['Socialicon']['id']));
 				...
 				?>
					<span class="col-md-2 success_msg<?php echo h($page['Socialicon']['id']); ?>"></span>
				</td>
                <td class="actions">
...
$('.success_msg_order'+element.data('socialicon_id')).show();
				
			    setTimeout(function() { $('.success_msg_order'+element.data('socialicon_id')).hide(); }, 2000);
				  
				 $('.success_msg_order'+element.data('socialicon_id')).html('<h3><i class="text-success icon-check"></i></h3>');
                    }
                });
            });            
  </script> 
        ...
        $(document).on('change', '.ddStatus', function(e) {
            ...
            $.ajax({ 
                ...
                success: function(jsonResponse) {
                   ...
                   // alert(response.message);
					$('.success_msg'+element.data('socialicon_id')).show();
				
			    setTimeout(function() { $('.success_msg'+element.data('socialicon_id')).hide(); }, 2000);
				  
				 $('.success_msg'+element.data('socialicon_id')).html('<h3><i class="text-success icon-check"></i></h3>');
                    }
                });
            });            
  </script> 
