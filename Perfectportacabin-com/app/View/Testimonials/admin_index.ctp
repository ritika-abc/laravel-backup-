            ...
            <tbody>
              ...
              <tr>
				...
				echo $this->Form->select('active', $options, array('empty'=>false, 'name'=>'data[Testimonial][active]', 'default'=>$item['Testimonial']['active'], 'class'=>'form-control ddStatus', 'data-testimonial_id'=>$item['Testimonial']['id']));
 				...
 				?>
               <span class="col-md-2 success_msg<?php echo h($item['Testimonial']['id']); ?>"></span>
				</td>
                <td class="actions">
...
//alert(response.message);
					$('.success_msg'+element.data('testimonial_id')).show();
				
			    setTimeout(function() { $('.success_msg'+element.data('testimonial_id')).hide(); }, 2000);
				  
				 $('.success_msg'+element.data('testimonial_id')).html('<h3><i class="text-success icon-check"></i></h3>');
                    }
                });
            });            
  </script> 
