 ...
 */
	...
	if(!empty($blog['Blog']['meta_keyword'])) {	
		$meta_keyword = $blog['Blog']['meta_keyword'];
	}
	if(!empty($page['Blog']['meta_desc'])) {	
 ...
 */
   ...
   public function admin_change_blog_order()
    {
        if(!$this->request->is('AJAX'))
        {
            return $this->redirect(array('controller'=>'blogs', 'action'=>'index'));
        ...
        if(!$this->Blog->save(array('id'=>$this->request->data['blog_id'], 'blog_order'=>$this->request->data['blog_order'])))
        {
            echo json_encode(array('status'=>'failure', 'message'=>'Unable to update Blog order at the moment.'));
            die;
        }
        
        echo json_encode(array('status'=>'success', 'message'=>'Blog Order updated.'));
        die;
    }
