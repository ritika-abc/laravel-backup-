...
////////////////////////////////////////////////////////////
	...
	public function productdetails($slug = null) {
		...
		$category_data = $this->Category->find('first', [
            'recursive' => 0,				 		
            'conditions' =>  array(
			'Category.slug' => $slug)
        ]);
		
		//$categoryname = $category_data['Category']['slug']; 
		
		  $directory_name = str_replace($this->webroot,'',$this->here);
		 // $directory_name = str_replace('/'.$categoryname,'',$directory_name);
    ...
    $this->set(compact('country','location','product','products','catalog'));
	 ...
	 public function product($slug=null) {
		...
		$directory_name = str_replace($this->webroot,'',$this->here);
		$directory_name = str_replace($product_slug,'',$directory_name);
 		$directory_name = str_replace('.html','',$directory_name);
